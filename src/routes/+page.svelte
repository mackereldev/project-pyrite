...
<div class="flex flex-row gap-8">
    <div class="flex flex-col gap-2 justify-center">
        ...
        <form on:submit|preventDefault={joinRoom} class="flex flex-row w-48 h-fit overflow-clip rounded-lg outline outline-1 outline-offset-0 outline-zinc-400 transition-all focus-within:outline-violet-500 focus-within:outline-2">
            <Input minlength={JOIN_CODE_LENGTH} maxlength={JOIN_CODE_LENGTH} min="0" bind:element={joinInput} bind:value={joinCode} on:input={validateJoinCodeNumerical} placeholder="Enter Code" required omitBorderStyle class="flex-grow w-full h-9" />
